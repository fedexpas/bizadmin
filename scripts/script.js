/*$( ".tiponuevo" )
  ...
  $('.equis').click(function(){
     ...
     var precio_descuento = $('.descuento_'+compra_id+'_'+producto_id).data('value');
     
     if (precio_descuento != 0) { // Si tiene descuento se muestra la fila: Total (sin descuento)
         var resta = precio_producto-precio_descuento; // En la linea 38 se descuenta del precio total el precio del producto con descuento
         
         var precio_sin_descuento = $('#precio_sin_descuento').data('value');
         $('#precio_sin_descuento').html('<b>$ '+(precio_sin_descuento-precio_producto)+'</b>');
         $('#precio_sin_descuento').data('value', precio_sin_descuento-precio_producto);
     }
     else { // No tiene descuento
         var resta = precio_producto; // En la linea 38 se descuenta el precio del producto (sin descuento porque no tiene)
     }
     
     var precio_total = $('#precio_total').data('value'); // Precio total del producto (inclye el descuento sie es que tiene)
     alert(precio_total-precio_producto);
     $('.tabla_'+compra_id+'_'+producto_id).hide();
     $('#precio_total').html('<b>$ '+(precio_total-resta)+'</b>');
     $('#precio_total').data('value', precio_total-resta);
