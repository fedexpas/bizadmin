...
class Producto extends CI_Controller {
        ...
        public function vender()
	{
        ...
        public function venta_ok()
        {
            ...
            $cantidad_iteracion = 1; // Si la cantidad de productos es mas de uno se ingresan varios registros a la tabla
            ...
            if ($this->session->userdata('compra_temp_ids') !== NULL) { // Si la variable de sesion compra_temp_ids existe procedemos a agregar un nuevo ID
                while ($datos_temp['cantidad'] >= $cantidad_iteracion) {
                    ...
                    $data['datos_venta']['compra_id'] = $compra_temp_id; // Para que sirve $data['datos_Venta'] 
                    $cantidad_iteracion++;
                }
            }
                ...
                while ($datos_temp['cantidad'] >= $cantidad_iteracion) {
                    $compra_temp_id = $this->compra_model->agregarCarrito($datos_temp);
                    $this->session->set_userdata(array('compra_temp_ids'=>$compra_temp_id.'-'.$datos_temp['producto_id'])); // @TODO: Si es la segunda iteracion tiene que agregar un ID, no borrar todo y empezar de nuevo
            ...
            if ($this->session->userdata('cliente_id') !== NULL) { // @TODO: Buscar las compras usando los datos de $this->session->userdata('compra_temp_ids')
                ...
                foreach ($compra_ids as $compra_id) {
                    $compra_id4real = explode('-', $compra_id); // $compra_id4real[0] tiene el verdadero id (de lo contrario usa la combinacion compra_id-producto_id)
                    $data['ventas'][$compra_id4real[0]] = $this->compra_model->getTempByCompraId($compra_id4real[0]); // cliente_compra_temp
                }
        ...
        public function del_producto_carrito()
        {
            ...
            if ($this->uri->segment(3) && $this->uri->segment(4)) {
                ...
                if(!$this->producto_model->borrar_carrito($compra_id, $producto_id)) {
                    ...
                    foreach ($compra_ids as $compra_id) {
                        if ($compra_id !== $ids_del) { // Si la combinacion compra_id-producto_id es distinta a la que acabamos de borrar entonces la agregamos 
                            $ids[$i] = $compra_id;
                            $i++;
                        }
                    }
